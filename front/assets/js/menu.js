...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (nomeUsuario) {
        saudacaoElemento.textContent = `Olá, ${nomeUsuario}`; // Exibe o nome do usuário
    } else {
        verificarSessao();
        saudacaoElemento.textContent = 'Olá, Usuário'; // Saudação padrão
    }

    // Adiciona a função de logout ao botão
    const logoutBtn = document.getElementById('logoutBtn'); // Certifique-se de que este ID está correto
    logoutBtn.addEventListener('click', async function () {
        try {
            ...
            if (data.success) {
                ...
                localStorage.removeItem('usuarioTipo'); // Remove o tipo de usuário também
